...
static enum TEST_METHOD test_method = RUN_REGULAR;
...
int main(int argc, char **argv)
{
    ...
    if (controller_ip &&
        !notify_controller(controller_ip, controller_port, connections, num_qps, WRITE_VERB)) {
        fprintf(stderr, "Fail to notify the controller\n");
    }

    if (test_method == RUN_INFINITELY) {
        if (!gen_req_traffic_infinitely(WRITE_VERB, connections, num_qps, tx_depth, use_event)) {
            fprintf(stderr, "Fail to generate traffic infinitely\n");
            goto destroy_socket;
        }
    } else if (barrier_sync) {
        if (!gen_req_barrier_sync_traffic(WRITE_VERB, connections, num_qps, iters, use_event)) {
            fprintf(stderr, "Fail to generate barrier synchronized traffic\n");
            goto destroy_socket;
        }
    } else if (!gen_req_traffic(WRITE_VERB, connections, num_qps, iters, tx_depth, use_event)) {
...
static void print_usage(char *app)
{
    ...
    fprintf(stderr, "      --run_infinitely         run test forever and print results every second\n");
...
static bool parse_args(int argc, char **argv)
{
    ...
    while (1) {
        ...
        switch (c) {
            ...
            // required for long options to work
            case 0:
                if (run_inf_flag) {
                    test_method = RUN_INFINITELY;
                }
                break;
